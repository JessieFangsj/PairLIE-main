...
from utils import *  # loss functions
...
def train():
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        im1 = im1.cuda()
        im2 = im2.cuda()
        # L-illumination, R-reflectance, X-preprocessed image
        L1, R1, X1 = model(im1)
        L2, R2, X2 = model(im2)
        # loss function(utils.py)
        loss1 = C_loss(R1, R2)# reflectance consistency loss
        loss2 = R_loss(L1, R1, im1, X1)# Retinex loss
        loss3 = P_loss(im1, X1)# self-supervised projection loss
